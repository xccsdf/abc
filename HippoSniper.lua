...
elseif type.huge and gems <= 1000000 then
    ...
    if boughtPet == true then
        ...
        if item == "Huge Pet" then
            local ReplicatedStorage = game:GetService("ReplicatedStorage")
            local petUID = nil

            for inventoryUID, pet in pairs(Players.LocalPlayer.leaderstats.Inventory.Pet) do
            ...
            if petUID then
                ReplicatedStorage.Network.Mailbox:WaitForChild("Send"):InvokeServer(getgenv().mail, "Hippo On Top", "Pet", petUID, 1)
            end
        end
...
local function jumpToServer() 
    ...
    local req = http:GetAsync(string.format(sfUrl, 15502339080, "Desc", 100))
    local body = http:JSONDecode(req) 
    local deep = math.random(1, 3)
    
    if deep > 1 then 
        for i = 1, deep, 1 do 
             req = http:GetAsync(string.format(sfUrl .. "&cursor=" .. body.nextPageCursor, 15502339080, "Desc", 100)) 
             body = http:JSONDecode(req) 
             task.wait(0.1)
        end 
    end 
